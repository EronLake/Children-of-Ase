...
void ShrineConfig::import_config(vector<WorldObj*>* recVec_ptr, unordered_map<Texture*, pair<string, int>>* textureMap, ChildrenOfOsi* gameplay_func, TaskBuffer* tBuffer)
{
	bool alive = true;
	Json::Value root;
		...
		//std::string test = 
		std::ifstream test("../ChildrenofOsi/ShrineConfig.json", std::ifstream::binary);
		std::string errs;
	...
	{
		set_shrine(recVec_ptr, gameplay_func, tBuffer, (*itr)["x"].asFloat(), (*itr)["y"].asFloat(),
...
void ShrineConfig::set_shrine(vector<WorldObj*>* recVec_ptr, ChildrenOfOsi* gameplay_func, TaskBuffer* tBuffer, float x, float y, float width,
	float hight, std::string name, float bodyx1, float bodyx2, float bodyy1, float bodyy2, int health)
{
	if (!Containers::npc_table[name]) {
		...
		Containers::npc_table[name]->set_npc_type(3);
	}
...
void ShrineConfig::init_sprites(ChildrenOfOsi* gameplay_func, TaskBuffer* tBuffer, unordered_map<Texture*,
