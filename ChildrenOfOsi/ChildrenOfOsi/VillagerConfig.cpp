...
void VillagerConfig::import_config(vector<WorldObj*>* recVec_ptr, unordered_map<Texture*, pair<string, int>>* textureMap, ChildrenOfOsi* gameplay_func, TaskBuffer* tBuffer)
{
	...
	while (alive) {
		...
		//std::string test = 
		std::ifstream test("../ChildrenofOsi/VillagerConfig.json", std::ifstream::binary);
		std::string errs;
	...
	{
			set_villager(recVec_ptr, gameplay_func, tBuffer, (*itr)["x"].asFloat(), (*itr)["y"].asFloat(),
...
void VillagerConfig::set_villager(vector<WorldObj*>* recVec_ptr, ChildrenOfOsi* gameplay_func, TaskBuffer* tBuffer, float x, float y, float width,
	float hight, std::string name, float bodyx1, float bodyx2, float bodyy1, float bodyy2, int health)
{
	...
	if (!Containers::npc_table[name]) {
		...
		Containers::npc_table[name]->set_npc_type(1);
	}
...
void VillagerConfig::init_sprites(ChildrenOfOsi* gameplay_func, TaskBuffer* tBuffer, unordered_map<Texture*,
